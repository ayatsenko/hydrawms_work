...
@RestController
@RequestMapping("/tenders_old")
public class TendersOLDController {
    ...
    public ModelAndView files_detail(@RequestParam(value="mode") Long mode,
                                     ...
                                     @RequestParam(value="tenders_files_table_order_column", required = false) Long order_column,
                                     @RequestParam(value="tenders_files_table_order_type", required = false) String order_type,
                                     @RequestParam(value="tenders_files_table_search", required = false) String table_search,
                                     @RequestParam(value="tenders_files_table_pagelen", required = false) Long pagelen,
                                     @RequestParam(value="tenders_files_table_page", required = false) Long page,

                                     @RequestParam(value="tenders_list_table_order_column", required = false) Long tenders_list_table_order_column,
                                     @RequestParam(value="tenders_list_table_order_type", required = false) String tenders_list_table_order_type,
                                     @RequestParam(value="tenders_list_table_search", required = false) String tenders_list_table_search,
                                     @RequestParam(value="tenders_list_table_pagelen", required = false) Long tenders_list_table_pagelen,
                                     @RequestParam(value="tenders_list_table_page", required = false) Long tenders_list_table_page
    ) {
        ...
        List<User_Role_List> user_Role_List;
        List<DetailTenderFilesList> detailTenderFilesList;
        ...
        if(mode == 0){
            ...
            detailTenderFilesList = detailTenderFilesListRepository.queryTenderFileDetailByID(
                    user_List.get(0).id, user_Role_List.get(0).role_id, req_store_id
            );

            mav.addObject("req_id", detailTenderFilesList.get(0).req_id);

            mav.addObject("req_name",  detailTenderFilesList.get(0).req_name);

            mav.addObject("req_store_id",  detailTenderFilesList.get(0).req_store_id);

            mav.addObject("req_store_date",  detailTenderFilesList.get(0).req_store_date);

            mav.addObject("user_id",  detailTenderFilesList.get(0).user_id);
            store_user_List = user_ListRepository.queryUserByID( detailTenderFilesList.get(0).user_id);
            mav.addObject("store_user_list", store_user_List);

            mav.addObject("req_store_filename",  detailTenderFilesList.get(0).req_store_filename);
        }

        mav.addObject("tenders_files_table_order_column", order_column);
        mav.addObject("tenders_files_table_order_type", order_type);
        mav.addObject("tenders_files_table_search", table_search);
        mav.addObject("tenders_files_table_pagelen", pagelen);
        mav.addObject("tenders_files_table_page", page);

        mav.addObject("tenders_list_table_order_column", tenders_list_table_order_column);
        mav.addObject("tenders_list_table_order_type", tenders_list_table_order_type);
        mav.addObject("tenders_list_table_search", tenders_list_table_search);
        mav.addObject("tenders_list_table_pagelen", tenders_list_table_pagelen);
        mav.addObject("tenders_list_table_page", tenders_list_table_page);

        mav.setViewName("/tenders/files_detail");
        return mav;
    }
